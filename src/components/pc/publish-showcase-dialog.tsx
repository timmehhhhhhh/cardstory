...
export function PublishShowcaseDialog({
  ...
  const [title, setTitle] = React.useState(pcName);
  ...
  if (open !== prevOpen) {
    setPrevOpen(open);
    if (open) {
      const entry = getShowcaseEntry(pcId);
  ...
  async function handlePublish() {
    ...
    try {
      ...
      const existing = getShowcaseEntry(pcId);
      ...
      if (existing) {
        ...
        if (res.ok) {
          ...
          setShowcaseEntry(pcId, { ...existing, publishedAt: new Date().toISOString() });
        }
      } else {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="bg-surface border-border sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Showcase this pc</DialogTitle>
          <DialogDescription>
