...
import { BinderSelector } from "@/app/portfolio/binder/_components/binder-selector";
import { LayoutPicker } from "@/app/portfolio/binder/_components/layout-picker";
import { BinderSpread, type VisiblePage } from "@/app/portfolio/binder/_components/binder-spread";
import { BinderPageNav } from "@/app/portfolio/binder/_components/binder-page-nav";
import { CardPickerSheet } from "@/app/portfolio/binder/_components/card-picker-sheet";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuTrigger,
...
export function BinderClient() {
  ...
  // PortfolioClient's active-portfolio reset).
  ...
  function handleRemovePage(pageId: string) {
    ...
    if (
      hasCards &&
      !window.confirm(
        "This page has cards placed. Remove it anyway? Your cards stay in your portfolio — only this binder arrangement is lost."
      )
    ) {
      return;
    }
  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-5 px-4 py-6 sm:px-6">
      ...
      <p className={cn("text-center text-xs text-muted-foreground", rows.length > 0 && "hidden")}>
        Your portfolio is empty — add cards from{" "}
