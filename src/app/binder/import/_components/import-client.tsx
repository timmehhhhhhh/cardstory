    ...
    try {
      ...
      for (const { holdingId, pocketIndex } of toApply) {
        try {
          ...
          // Defensive: the holding exists in PC even if this local write
          // failed (see plan's compensating strategy) — surfaced via the
          // same retry banner as a failed PC write, so "Retry" covers both.
